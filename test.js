...
//step 的参数是一系列函数，这些函数接收名为next的方法和额外参数作为参数
...
describe('并行序列测试', function() {
  ...
  it('包含多个方法', function(done) {
    let foo = step([f(300), f(100), f(400)]);
    foo(function(err, data) {
      assert.isNull(err, '没有错误抛出')
      assert.deepEqual(data, [
        [100, 300, 400]
...
describe('同时包含串行和并行方法序列', function() {
  it('测试一(包含同步方法)', function(done) {
    let foo = step(f(600), [f(220), f(0), f(680)], f(50), f(0), [f(1000)], f(240), f(180))
    foo(function(err, data) {
      assert.isNull(err, '没有错误抛出')
  ...
  it('测试二', function(done) {
    let foo = step(f(400), f(140), [f(380), f(20)], f(500), f(10), f(30), [f(320), f(80), f(40)], f(320), [f(20)], f(90))
    foo(function(err, data) {
      assert.isNull(err, '没有错误抛出')
      assert.deepEqual(data, [400, 140, [20, 380], 500, 10, 30, [40, 80, 320], 320, [20], 90], '结果正确')
      done()
    })
  })
})
...
describe('测试错误捕获', function() {
  it('在串行中抛出错误', function(done) {
    let foo = step(f(600), [f(220), f(120), f(680)], f(50), function(next, payload) {
      setTimeout(function() {
        try {
          throw new Error(100);
          next(null, payload);
  ...
  it('在并行中抛出错误', function(done) {
    let foo = step(f(600), [f(220), function(next, payload) {
      setTimeout(function() {
        try {
          throw new Error(120);
          next(null, payload);
  ...
  function f(time, err = null) {
    return function(next, payload) {
      setTimeout(function() {
        ...
        next(err, time + payload);
      }, time);
    }
  }
  it('将返回的结果相加并传递', function(done) {
    let foo = step(f(60), f(40), [f(220), f(10), f(280)], f(50), [f(500), f(90)], f(240), f(180))
    foo(function(err, data) {
      assert.isNull(err, '没有错误抛出')
      assert.deepEqual(data, [
